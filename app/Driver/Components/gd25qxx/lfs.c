...
{
    a->tag ^= LFS_MKTAG(0x7ff, 0x3ff, 0) & (b->tag ^ ((id != 0x3ff) ? LFS_MKTAG(LFS_TYPE_DELETE, id, 0) : 0));
                         ...
                         lfs_mdir_t *dir, const lfs_block_t pair[2])
{
    ...
    return (int)lfs_dir_fetchmatch(lfs, dir, pair, ~(lfs_tag_t)0, 0, NULL, NULL, NULL);
}
...
int lfs_mkdir(lfs_t *lfs, const char *path)
{
    ...
    lfs_pair_tole32(dir.pair);
    err = lfs_dir_commit(lfs, &cwd, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_CREATE, id, 0), NULL}, {LFS_MKTAG(LFS_TYPE_DIR, id, nlen), path}, {LFS_MKTAG(LFS_TYPE_DIRSTRUCT, id, 8), dir.pair}, {!cwd.split ? LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 8) : LFS_MKTAG(LFS_FROM_NOOP, 0, 0), dir.pair}));
    lfs_pair_fromle32(dir.pair);
    if (err)
    {
        LFS_TRACE("lfs_mkdir -> %d", err);
...
int lfs_rename(lfs_t *lfs, const char *oldpath, const char *newpath)
{
    ...
    err = lfs_dir_commit(lfs, &newcwd, LFS_MKATTRS({prevtag != LFS_ERR_NOENT ? LFS_MKTAG(LFS_TYPE_DELETE, newid, 0) : LFS_MKTAG(LFS_FROM_NOOP, 0, 0), NULL}, {LFS_MKTAG(LFS_TYPE_CREATE, newid, 0), NULL}, {LFS_MKTAG(lfs_tag_type3(oldtag), newid, strlen(newpath)), newpath}, {LFS_MKTAG(LFS_FROM_MOVE, newid, lfs_tag_id(oldtag)), &oldcwd}));
    if (err)
    {
        LFS_TRACE("lfs_rename -> %d", err);
        return err;
...
static int lfs_init(lfs_t *lfs, const struct lfs_config *cfg)
{
    ...
    LFS_ASSERT(4 * lfs_npw2(LFS_BLOCK_NULL / (lfs->cfg->block_size - 2 * 4)) <= lfs->cfg->block_size);
                           ...
                           const lfs_block_t oldpair[2], lfs_block_t newpair[2])
{
    ...
    if (err != LFS_ERR_NOENT)
    {
        ...
        lfs_pair_tole32(newpair);
        err = lfs_dir_commit(lfs, &parent, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_TAIL + parent.split, 0x3ff, 8), newpair}));
        lfs_pair_fromle32(newpair);
        if (err)
        {
            return err;
...

    {
        ...
        while (!lfs_pair_isnull(dir1.d.tail))
        {
            ...
            while (true)
            {
                ...
                err = lfs_dir_commit(lfs, &dir2, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_CREATE, id, 0), NULL}, {LFS_MKTAG(isdir ? LFS_TYPE_DIR : LFS_TYPE_REG, id, entry1.d.nlen), name}, {LFS_MKTAG(isdir ? LFS_TYPE_DIRSTRUCT : LFS_TYPE_CTZSTRUCT, id, sizeof(&entry1.d.u)), &entry1.d.u}));
            ...
            if (!lfs_pair_isnull(dir1.d.tail))
            {
                ...
                err = lfs_dir_commit(lfs, &dir2, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 0), dir1.d.tail}));
